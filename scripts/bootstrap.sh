...
MCP_ENABLED=0
ENV_EOF
  echo "WARNING: Missing .env in S3 config bucket and no existing ${ENV_PATH}; wrote minimal runtime defaults with MCP disabled. Provider-backed analysis stays unavailable until a real .env is uploaded." >&2
fi
chmod 600 "${ENV_PATH}"
